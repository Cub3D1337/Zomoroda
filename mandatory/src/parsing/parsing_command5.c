...
int	check_column_top_bottom(t_config *cfg, int x)
{
	if (x >= cfg->map_w)
		return (1);
	if (cfg->map[0][x] == '0' || ft_strchr("NSEW", cfg->map[0][x]))
		return (error("Error\nMap not closed at top edge\n"));
	if (cfg->map[cfg->map_h - 1][x] == '0' || ft_strchr("NSEW",
...
int	check_row_sides(t_config *cfg, int y)
{
	if (y >= cfg->map_h)
		return (1);
	if (cfg->map[y][0] == '0' || ft_strchr("NSEW", cfg->map[y][0]))
		return (error("Error\nMap not closed at left edge\n"));
	if (cfg->map[y][cfg->map_w - 1] == '0' || ft_strchr("NSEW",
