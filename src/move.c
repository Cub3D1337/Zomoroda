...
/*   Updated: 2025/08/06 10:50:47 by abnsila          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	check_boundaries(t_cub *cub)
{
	...
	ply_corners[0].x = (cub->p.pos.x - cub->p.half) / MAP_SIZE; // Top-Left
	ply_corners[0].y = (cub->p.pos.y - cub->p.half) / MAP_SIZE;
	ply_corners[1].x = (cub->p.pos.x + cub->p.half - 1) / MAP_SIZE; // Top-Right
	ply_corners[1].y = (cub->p.pos.y - cub->p.half) / MAP_SIZE;
	ply_corners[2].x = (cub->p.pos.x - cub->p.half) / MAP_SIZE; // Bottom-Left
	ply_corners[2].y = (cub->p.pos.y + cub->p.half - 1) / MAP_SIZE;
	ply_corners[3].x = (cub->p.pos.x + cub->p.half - 1) / MAP_SIZE; // Bottom-Right
...
void move(t_cub *cub)
{
	...
	{
		cub->p.prev_pos.y = cub->p.pos.y;
		cub->p.prev_pos.x = cub->p.pos.x;
		delta.x = 0.0;
		delta.y = 0.0;
